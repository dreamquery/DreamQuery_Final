...
class mcDataParser {
  ...
  public function txtParsingEngine($text, $admin = true) {
    ...
    if ($this->settings->enableBBCode == 'yes' || $admin) {
      return msDataParser::wordWrap($this->bbCode->bbParser($text));
    } else {
      return msDataParser::wordWrap(mc_NL2BR(msDataParser::autoLinkParser($text)));
    }
  }
  ...
  public function wordWrap($text) {
    if ($this->wordwrap[0] == 'on') {
      return wordwrap($text, $this->wordwrap[1], mc_defineNewline(), true);
    }
    return $text;
  }
