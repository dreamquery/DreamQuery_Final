...
$headerTitleText = mc_cleanData($public_accounts[12] . ': ' . $headerTitleText);

// Breadcrumb..
$breadcrumbs[] = '<a href="' . $MCRWR->url(array('account')) . '">' . $public_accounts[8] . '</a>';
$breadcrumbs[] = $public_accounts[12];
...
// Pagination..
$pgn = array(
  'limit' => $page * SAVED_SEARCHES_PER_PAGE - (SAVED_SEARCHES_PER_PAGE),
  'per' => SAVED_SEARCHES_PER_PAGE
);

$countRows = $MCACC->searches($loggedInUser['id'], '', $pgn, 'yes');
if ($countRows > $pgn['per']) {
  if ($SETTINGS->en_modr == 'yes') {
    $seolink  = $MCRWR->url(array($MCRWR->config['slugs']['ssc'] . '/{page}'));
    $pNumbers = mc_publicPageNumbers($countRows, $pgn['per'], $seolink);
  } else {
    $pNumbers = mc_publicPageNumbers($countRows, $pgn['per'], $SETTINGS->ifolder . '/?p=saved-searches&amp;next=');
  }
}
...
$tpl->assign('TEXTW', $public_accounts_saved);
...
$tpl->assign('SAVED_SEARCHES', $MCACC->searches($loggedInUser['id'], $public_accounts_saved, $pgn));
$tpl->assign('PAGINATION', ($countRows > 0 && isset($pNumbers) ? $pNumbers : ''));

// Global..
include(PATH . 'control/system/global.php');

$tpl->display(THEME_FOLDER . '/account-saved-searches.tpl.php');
