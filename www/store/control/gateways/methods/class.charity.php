...
class charity extends paymentHandler {
  ...
  public function gatewayFields($ssl, $buyCode, $id, $itemName) {
    ...
    $fields  = array(
      'merchantID' => ($this->settings->gatewayMode == 'live' ? $params['merchant-id'] : '0000992'),
      ...
      'callbackURL' => $url . 'checkout/charity.php',
    ...
    // Create signature..
    $fields['signature'] = charity::charitySig($fields, $params['pre-share-key'], $id);
    return array(
      'form',
      $fields
    );
  ...
  // Calculate signature..
  private function charitySig($data, $key, $id) {
    $ret  = '';
