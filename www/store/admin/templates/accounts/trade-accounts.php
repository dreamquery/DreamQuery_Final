...
$query = mysqli_query($GLOBALS["___msw_sqli"], "SELECT SQL_CALC_FOUND_ROWS *,
         ...
         (SELECT count(*) FROM `" . DB_PREFIX . "sales`
          ...
          AND `" . DB_PREFIX . "sales`.`type` = 'trade'
         ) AS `saleCount`,
...
<div class="fieldHeadWrapper">
  <p>
   <span class="float">
    <a href="?p=add-account&amp;loadtype=trade"><i class="fa fa-plus fa-fw"></i></a>
    <?php
    if ($countedRows > 0) {
    ?>
    ...
    <a href="?p=taccounts&amp;export=<?php echo (isset($_GET['orderby']) ? mc_safeHTML($_GET['orderby']) : 'all') . (isset($_GET['keys']) ? '&amp;keys=' . mc_safeHTML($_GET['keys']) : ''); ?>"><i class="fa fa-save fa-fw"></i></a>
    <?php
    }
    ?>
   </span><?php echo $msg_admin3_0[50]; ?> (<?php echo @number_format($countedRows); ?>):
  </p>
</div>
...
<div class="formFieldWrapper" id="filters" style="display:none">
  <select name="orderby" onchange="if(this.value!=0){location=this.options[this.selectedIndex].value}">
    <option value="?p=taccounts&amp;orderby=name_asc"<?php echo (isset($_GET['orderby']) && $_GET['orderby'] == 'name_asc' ? ' selected="selected"' : ''); ?>><?php echo $msg_accounts19; ?></option>
    <option value="?p=taccounts&amp;orderby=name_desc"<?php echo (isset($_GET['orderby']) && $_GET['orderby'] == 'name_desc' ? ' selected="selected"' : ''); ?>><?php echo $msg_accounts20; ?></option>
    <option value="?p=taccounts&amp;orderby=email_asc"<?php echo (isset($_GET['orderby']) && $_GET['orderby'] == 'email_asc' ? ' selected="selected"' : ''); ?>><?php echo $msg_accounts21; ?></option>
    <option value="?p=taccounts&amp;orderby=email_desc"<?php echo (isset($_GET['orderby']) && $_GET['orderby'] == 'email_desc' ? ' selected="selected"' : ''); ?>><?php echo $msg_accounts22; ?></option>
    <option value="?p=taccounts&amp;orderby=orders_asc"<?php echo (isset($_GET['orderby']) && $_GET['orderby'] == 'orders_asc' ? ' selected="selected"' : ''); ?>><?php echo $msg_accounts23; ?></option>
    <option value="?p=taccounts&amp;orderby=orders_desc"<?php echo (isset($_GET['orderby']) && $_GET['orderby'] == 'orders_desc' ? ' selected="selected"' : ''); ?>><?php echo $msg_accounts24; ?></option>
    <option value="?p=taccounts&amp;orderby=revenue_asc"<?php echo (isset($_GET['orderby']) && $_GET['orderby'] == 'revenue_asc' ? ' selected="selected"' : ''); ?>><?php echo $msg_accounts25; ?></option>
    <option value="?p=taccounts&amp;orderby=revenue_desc"<?php echo (isset($_GET['orderby']) && $_GET['orderby'] == 'revenue_desc' ? ' selected="selected"' : ''); ?>><?php echo $msg_accounts26; ?></option>
  </select>
</div>
...
<div class="formFieldWrapper" id="filters2" style="display:none">
  ...
  <p>
  <input type="hidden" name="p" value="taccounts"><input type="text" name="keys" class="box" placeholder="<?php echo mc_cleanDataEntVars($msg_accounts28); ?>" value="<?php echo (isset($_GET['keys']) ? mc_safeHTML($_GET['keys']) : ''); ?>">
  <input style="margin-top:10px" type="submit" class="btn btn-primary" value="<?php echo mc_cleanDataEntVars($msg_accounts27); ?>">
  <input style="margin:10px 0 0 20px" type="button" onclick="window.location='?p=taccounts'" class="btn btn-success" value="<?php echo mc_cleanDataEntVars($msg_script11); ?>" title="<?php echo mc_cleanDataEntVars($msg_script11); ?>">
  </p>
  </form>
</div>

<div id="formField">
<form method="post" action="?p=taccounts">
<?php
if ($countedRows > 0) {
...
<div class="panel panel-default" id="accarea_<?php echo $ACCOUNTS->id; ?>">
  <div class="panel-body">
    ...
    <?php echo $msg_accounts12; ?>: <a href="?p=taccounts&amp;accstatus=<?php echo $ACCOUNTS->id; ?>" onclick="mc_Window(this.href,'<?php echo DIVWIN_NOTES_HEIGHT; ?>','<?php echo DIVWIN_NOTES_WIDTH; ?>',this.title);return false;"><?php echo ($ACCOUNTS->enabled == 'yes' ? $msg_accounts13 : $msg_accounts14); ?></a><br>
    <?php echo $msg_accounts15; ?>: <?php echo ($ACCOUNTS->ip ? $ACCOUNTS->ip : 'N/A'); ?><br>
    <?php echo $msg_accounts35; ?>: <?php echo $ACCOUNTS->tradediscount; ?>%<br>
    <?php echo $msg_accounts36; ?>: <?php echo $ACCOUNTS->minqty; ?><br>
    <?php echo $msg_accounts37; ?>: <?php echo $ACCOUNTS->maxqty; ?><br>
    <?php echo $msg_accounts47; ?>: <?php echo ($ACCOUNTS->mincheckout > 0 ? mc_currencyFormat(mc_formatPrice($ACCOUNTS->mincheckout)) : 'N/A') ?><br>
    <?php echo $msg_accounts38; ?>: <?php echo $ACCOUNTS->stocklevel; ?>
    <hr>
    <a href="?p=taccounts&amp;message=<?php echo $ACCOUNTS->id; ?>" onclick="mc_Window(this.href,'<?php echo DIVWIN_NOTES_HEIGHT; ?>','<?php echo DIVWIN_NOTES_WIDTH; ?>',this.title);return false;"><i class="fa fa-bullhorn fa-fw"></i> <?php echo $msg_accounts29; ?></a>
    </div>
  </div>
  <div class="panel-footer">
    ...
    <a href="?p=taccounts&amp;notes=<?php echo $ACCOUNTS->id; ?>" onclick="mc_Window(this.href,'<?php echo DIVWIN_NOTES_HEIGHT; ?>','<?php echo DIVWIN_NOTES_WIDTH; ?>',this.title);return false;" title="<?php echo mc_safeHTML($msg_accounts8); ?>"><i class="fa fa-file-text<?php echo ($ACCOUNTS->notes == null || $ACCOUNTS->notes=='' ? '-o' : ''); ?> fa-fw"></i></a>
