...
$key  = substr(md5(uniqid(rand(),1)),3,30);
$key .= substr(md5(uniqid(rand(),1)),3,20);
?>

<form method="post" id="form" action="?p=add-manual" onsubmit="return confirmMessage_Add('<?php echo mc_cleanDataEntVars($msg_javascript45); ?>')">
<div id="content">
<script>
//<![CDATA[
...
<div class="formFieldWrapper">
  <div class="formLeft">
    ...
    $cats[] = $CATS->id;
    ?>
    <option value="?p=add-manual&amp;type=<?php echo $_GET['type']; ?>&amp;catid=<?php echo $CATS->id; ?>"<?php echo (count($cats)==1 || isset($_GET['catid']) && $_GET['catid']==$CATS->id ? ' selected="selected"' : ''); ?>><?php echo mc_safeHTML($CATS->catname); ?></option>
    <?php
    ...
    ?>
    <option value="?p=add-manual&amp;type=<?php echo $_GET['type']; ?>&amp;catid=<?php echo $CHILDREN->id; ?>"<?php echo (isset($_GET['catid']) && $_GET['catid']==$CHILDREN->id ? ' selected="selected"' : ''); ?>>&nbsp;&nbsp;<?php echo mc_safeHTML($CHILDREN->catname); ?></option>
    <?php
    ...
    ?>
    <option value="?p=add-manual&amp;type=<?php echo $_GET['type']; ?>&amp;catid=<?php echo $INFANTS->id; ?>"<?php echo (isset($_GET['catid']) && $_GET['catid']==$INFANTS->id ? ' selected="selected"' : ''); ?>>&nbsp;&nbsp;&nbsp;&nbsp;<?php echo mc_safeHTML($INFANTS->catname); ?></option>
    <?php
    }
    }
    }
...
<p style="text-align:center;padding-top:20px">
 ...
 <input type="hidden" name="pCat" value="<?php echo (isset($_GET['catid']) ? mc_digitSan($_GET['catid']) : '0'); ?>">
 ...
 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input class="btn btn-success" type="button" onclick="window.location='?p=sales-add'" value="<?php echo mc_cleanDataEntVars($msg_script11); ?>" title="<?php echo mc_cleanDataEntVars($msg_script11); ?>">
</p>
<?php
}
}
