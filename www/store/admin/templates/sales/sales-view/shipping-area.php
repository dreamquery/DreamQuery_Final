      ...
      ?>
      <option value="<?php echo $CTY->cnt_id; ?>"<?php echo ($SALE->shipSetCountry==$CTY->cnt_id ? ' selected="selected"' : ''); ?>><?php echo mc_safeHTML($CTY->cName); ?></option>
      <?php
      }
      ?>
      </select>
      ...
      ?>
      <option value="<?php echo $ZAREA->id; ?>"<?php echo ($SALE->shipSetArea==$ZAREA->id ? ' selected="selected"' : ''); ?>><?php echo mc_safeHTML($ZAREA->areaName); ?></option>
      <?php
      }
      if (mysqli_num_rows($q_zone)>0) {
      ?>
      ...
      ?>
      <option value="qtyr<?php echo $QR->id; ?>"<?php echo ($SALE->setShipRateID==$QR->id && $SALE->shipType=='qtyr' ? ' selected="selected"' : ''); ?>>&nbsp;<?php echo $QR->qtyFrom.' - '.$QR->qtyTo.' ('.(substr($QR->rate,-1) == '%' ? $QR->rate : mc_currencyFormat(mc_formatPrice($QR->rate))) . ')'; ?></option>
      <?php
      }
      }
      //---------------------------------------------------------------------
      ...
      <input type="text" class="box" name="cartWeight" id="cartWeight" value="<?php echo $SALE->cartWeight; ?>">
