...
<div class="fieldHeadWrapper">
  ...
  echo $msg_javascript439; ?> <a href="?p=shipping"><i class="fa fa-cog fa-fw"></i></a>:</p>
</div>
...
<div class="formFieldWrapper">
  <div class="formLeft">
    ...
    <input tabindex="<?php echo (++$tabIndex); ?>" type="text" name="priceFrom" value="<?php echo (isset($EDIT->priceFrom) ? $EDIT->priceFrom : '0.00'); ?>" class="box">
    <input style="margin-top:5px" tabindex="<?php echo (++$tabIndex); ?>" type="text" name="priceTo" value="<?php echo (isset($EDIT->priceTo) ? $EDIT->priceTo : '0.00'); ?>" class="box">

    <label style="margin-top:10px"><?php echo $msg_percrates15; ?> (%): <?php echo mc_displayHelpTip($msg_javascript445,'LEFT'); ?></label>
    <input tabindex="<?php echo (++$tabIndex); ?>" type="text" name="percentage" value="<?php echo (isset($EDIT->percentage) ? mc_rateCleaner($EDIT->percentage) : '0'); ?>" class="box">
...
<div class="formFieldWrapper" id="filters" style="display:none">
  ...
  <option value="?p=percent"><?php echo $msg_percrates18; ?></option>
  <?php
  $q_zones = mysqli_query($GLOBALS["___msw_sqli"], "SELECT *,`" . DB_PREFIX . "zones`.`id` AS `zid` FROM `" . DB_PREFIX . "zones`
             LEFT JOIN `" . DB_PREFIX . "countries`
             ON `" . DB_PREFIX . "zones`.`zCountry` = `" . DB_PREFIX . "countries`.`id`
...
$q_rates = mysqli_query($GLOBALS["___msw_sqli"], "SELECT SQL_CALC_FOUND_ROWS *,`" . DB_PREFIX . "percent`.`id` AS `pid`,`" . DB_PREFIX . "zones`.`id` AS `zid`
...
if (mysqli_num_rows($q_rates)>0) {
  ...
  <div class="panel panel-default">
    <div class="panel-body">
      ...
      <?php echo mc_currencyFormat($RATES->priceFrom); ?> - <?php echo mc_currencyFormat($RATES->priceTo); ?> @ <?php echo $RATES->percentage; ?>%
    </div>
    <div class="panel-footer">
      <a href="?p=percent&amp;edit=<?php echo $RATES->pid.(isset($_GET['service']) ? '&amp;service='.mc_digitSan($_GET['service']) : ''); ?>"><i class="fa fa-pencil fa-fw"></i></a><?php echo ($uDel=='yes' ? ' <a href="?p=percent&amp;del='.$RATES->pid.'" onclick="return mc_confirmMessage(\''.mc_filterJS($msg_javascript45).'\')"><i class="fa fa-times fa-fw mc-red"></i></a>' : ''); ?>
    </div>
  </div>
  <?php
  }
