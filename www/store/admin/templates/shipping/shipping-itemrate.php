...
$flZones   = mc_getFlatRateZones('per');
$cnt_rates = mc_rowCount('per');
?>
<div id="content">
...
if (isset($OK)) {
  if ($run[1]>0) {
    echo mc_actionCompleted(str_replace(array('{count}','{count2}'),array($run[0],$run[1]),$msg_itemrates8));
  } else {
    echo mc_actionCompleted(str_replace('{count}',$run[0],$msg_itemrates8));
  }
}
if (isset($OK2)) {
  echo mc_actionCompleted($msg_itemrates11);
}
if (isset($OK3) && $cnt>0) {
  echo mc_actionCompleted($msg_itemrates10);
}
...
<div class="fieldHeadWrapper">
  ...
  <span style="float:right">
    <a href="#" onclick="jQuery('#addArea').hide();jQuery('#enabArea').show();return false" class="enableDisable" title="<?php echo mc_cleanDataEntVars($msg_itemrates20); ?>"><i class="fa fa-check-square-o fa-fw"></i></a>
  </span>
  <?php
  }
  echo $msg_javascript577; ?> <a href="?p=shipping"><i class="fa fa-cog fa-fw"></i></a>:</p>
</div>
...
<div class="formFieldWrapper">
  <div class="formLeft">
    ...
    <input tabindex="<?php echo (++$tabIndex); ?>" type="text" name="rate" value="<?php echo (isset($EDIT->rate) ? $EDIT->rate : '0.00'); ?>" class="box">
	  ...
	  <label style="margin-top:10px"><?php echo $msg_itemrates22; ?> (<?php echo $msg_itemrates24; ?>): <?php echo mc_displayHelpTip($msg_javascript579,'LEFT'); ?></label>
    <input tabindex="<?php echo (++$tabIndex); ?>" type="text" name="item" value="<?php echo (isset($EDIT->item) ? $EDIT->item : '0.00'); ?>" class="box">

    <label style="margin-top:10px"><?php echo $msg_itemrates13; ?>: <?php echo mc_displayHelpTip($msg_javascript443,'LEFT'); ?></label>
...
<div class="formFieldWrapper">
  <div class="formLeft">
    ...
    ?>
    <input type="checkbox" name="log" id="log2" value="all" onclick="mc_toggleCheckBoxes(this.checked,'categoryBoxes')" tabindex="<?php echo (++$tabIndex); ?>"> <b><?php echo $msg_itemrates5; ?></b><br>
    <?php
    while ($ZONES = mysqli_fetch_object($q_zones)) {
    ?>
...
<p style="text-align:center;padding-top:20px">
  ...
  <input class="btn btn-primary" type="submit" value="<?php echo mc_safeHTML((isset($EDIT->id) ? $msg_itemrates9 : $msg_itemrates3)); ?>" title="<?php echo mc_safeHTML((isset($EDIT->id) ? $msg_itemrates9 : $msg_itemrates3)); ?>"><?php echo (isset($EDIT->id) ? '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input class="btn btn-success" type="button" onclick="window.location=\'?p=itemrate\'" value="'.mc_cleanDataEntVars($msg_script11).'" title="'.mc_cleanDataEntVars($msg_script11).'">' : ''); ?>
</p>
</form>
</div>
...
<div class="formFieldWrapper" id="zA_update">
  <div class="formLeft">
    ...
    $q_zones = mysqli_query($GLOBALS["___msw_sqli"], "SELECT *,`" . DB_PREFIX . "zones`.`id` AS `zid` FROM `" . DB_PREFIX . "zones`
               ...
               ") or die(mc_MySQLError(__LINE__,__FILE__));
    ?>
    <input type="checkbox" name="log" id="log" value="all" onclick="mc_toggleCheckBoxes(this.checked,'categoryBoxes')" tabindex="<?php echo (++$tabIndex); ?>"> <b><?php echo $msg_itemrates5; ?></b><br>
    <?php
    while ($ZONES = mysqli_fetch_object($q_zones)) {
    ?>
    <input tabindex="<?php echo (++$tabIndex); ?>" type="checkbox" name="inZone[]" value="<?php echo $ZONES->zid; ?>"> <?php echo mc_cleanData($ZONES->cName); ?> - <?php echo mc_safeHTML($ZONES->zName); ?><br>
    ...
    <label style="margin-top:10px"><?php echo $msg_itemrates13; ?>:</label>
...
<p style="text-align:center;padding-top:20px">
  ...
  <input class="btn btn-primary" type="submit" value="<?php echo mc_cleanDataEntVars($msg_itemrates21); ?>" title="<?php echo mc_cleanDataEntVars($msg_itemrates21); ?>">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
$q_rates = mysqli_query($GLOBALS["___msw_sqli"], "SELECT SQL_CALC_FOUND_ROWS *,`" . DB_PREFIX . "per`.`id` AS `fid`,`" . DB_PREFIX . "zones`.`id` AS `zid`
           FROM `" . DB_PREFIX . "per`
...
if (mysqli_num_rows($q_rates)>0) {
  ...
  <div class="panel panel-default">
    <div class="panel-body">
      ...
      <?php echo str_replace(array('{rate}','{item}'),array(mc_currencyFormat($RATES->rate),mc_currencyFormat($RATES->item)),$msg_itemrates23); ?>
    </div>
    <div class="panel-footer">
      <a href="?p=itemrate&amp;edit=<?php echo $RATES->fid.(isset($_GET['service']) ? '&amp;service='.mc_digitSan($_GET['service']) : ''); ?>"><i class="fa fa-pencil fa-fw"></i></a><?php echo ($uDel=='yes' ? ' <a href="?p=itemrate&amp;del='.$RATES->fid.'" onclick="return mc_confirmMessage(\''.mc_filterJS($msg_javascript45).'\')"><i class="fa fa-times fa-fw mc-red"></i></a>' : ''); ?>
    </div>
  </div>
  <?php
  }
