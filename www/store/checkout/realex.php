...
if ($INCOMING['code-id']) {
  ...
  if ($SALE_ID > 0) {
    $meta = $SETTINGS->ifolder . '/index.php?gw=' . $SALE_ID . '-' . $SALE_CODE . '&amp;count=1';
    $css  = '<link rel="stylesheet" href="' . $SETTINGS->ifolder . '/' . THEME_FOLDER . '/css/stylesheet.css" type="text/css">';
    echo '<html><head><meta http-equiv="refresh" content="1;url=' . $meta . '"><title></title>' . $css . '</head>';
    echo '<body class="realex_body"><p class="realex_redirect">' . str_replace(array(
      '{website}',
      '{url}'
    ), array(
      mc_safeHTML($SETTINGS->website),
      $meta
    ), $public_checkout132) . '</p></body>';
    echo '</html>';
    exit;
  }
